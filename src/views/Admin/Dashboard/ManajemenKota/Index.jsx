...
export default function ManajemenKota() {
    ...
    const { data: kotas, isLoading, refetch: refetchKotas } = useKotas({
        onError: (err) => {
            console.error(err)
            setMessage(err?.response?.data?.message)
        },
    ...
    return (
        <>
            ...
            {isLoading || deleteIsLoading ? (
                ...
                <>
                    <section>
                        <h3 className="text-2xl text-balck font-medium">{(logged.role == 'Superadmin' || logged.role == 'Admin Create') ? 'Manajemen Kota' : 'Kota'}</h3>
                    </section>

                    <section className="flex flex-col gap-5">
                        {(logged.role == 'Superadmin' || logged.role == 'Admin Create') && (
                        ...
                        <div className="flex flex-col overflow-x-auto shadow-md sm:rounded-lg gap-4">
                            <Table title={(isLogged().role === 'Superadmin' || isLogged().role === 'Admin Create')
                                ? ['No', 'Kota', 'Action']
                                : ['No', 'Kota']
                            } datas={kotas?.data.data} row={['kota']} links={{ edit: '/admin/dashboard/manajemen-kota/kota-edit/' }} emitDelete={handleDelete} />
                        </div>
                    </section>
                </>
            )}
